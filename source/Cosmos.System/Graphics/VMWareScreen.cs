...
namespace Cosmos.System.Graphics
{
    class VMWareScreen : Canvas
    {
        ...
        public VMWareScreen() : base()
        {
            Global.mDebugger.SendInternal($"Creating new VMWareScreen() with default mode {defaultGraphicMode}");

            // We don't need to add a Control for defaultGraphicMode we assume it to be always a valid mode
            driver = new VMWareSVGAII();
            driver.SetMode((ushort)defaultGraphicMode.Columns, (ushort)defaultGraphicMode.Rows, (ushort)defaultGraphicMode.ColorDepth);
        }

        public VMWareScreen(Mode mode) : base(mode)
        {
            ...
            driver = new VMWareSVGAII();
            driver.SetMode((ushort)mode.Columns, (ushort)mode.Rows, (ushort)mode.ColorDepth);
        }
        ...
        public override void DrawPoint(Pen pen, int x, int y)
        {
            ...
            switch (mode.ColorDepth)
            {
                case ColorDepth.ColorDepth32:
                    ...
                    Global.mDebugger.SendInternal($"Drawing Point of color {color} at pos {{{x},{y}}}");
